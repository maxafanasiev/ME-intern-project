...
async def update_user(user_id: int, user: UserUpdateRequestModel,
    ...
    db_user = await user_service.update_user(user_id, user, current_user)
    return db_user
...
async def delete_user(user_id: int, user_service: Annotated[UserService, Depends(user_service)],
                      current_user: User = Depends(app_service.get_current_user)):
    db_user = await user_service.delete_user(user_id, current_user)
    return db_user
...
async def read_users(
    ...
    result = await user_service.get_all_users(params.page, params.size)
    return {"users": result}
